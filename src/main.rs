...
use iced::widget::{
    button, checkbox, column, container, image, row, scrollable, text, text_input, Column, Space,
};
...
const AUDIO_EXTENSIONS: &[&str] = &[
    "mp3", "flac", "m4a", "m4b", "mp4", "ogg", "opus", "oga", "wav", "aiff", "aif", "aifc", "wv",
    "ape",
];
...
pub fn main() -> iced::Result {
    let arg_dir = std::env::args().nth(1).and_then(|arg| {
        if arg == "-h" || arg == "--help" {
...
impl Taguar {
    ...
    fn sidebar_view(&self) -> Element<'_, Message> {
        ...
        let field = |lbl: &'static str, val: &str, msg: fn(String) -> Message| -> Element<Message> {
            column![
                label(lbl),
        ...
        let save_row = row![
            ...
            text(self.status.as_deref().unwrap_or("")).size(11).color(MUTED),
        ]
        .spacing(10)
        .align_y(Alignment::Center);
        ...
        let play_glyph = if is_this_playing { "\u{23F8}" } else { "\u{25B6}" };
        let play_label = if is_this_playing { "Pause" } else { "Play" };
        let mut play_btn = button(
            row![
                text(play_glyph).size(14),
                text(play_label).size(12),
            ]
            .spacing(6)
            .align_y(Alignment::Center),
        )
        ...
        if let Some(v1) = &self.id3v1 {
            ...
            let v1_row =
                |lbl: &'static str, val: &str| -> Element<Message> {
                    row![
    ...
    fn status_bar_view(&self) -> Element<'_, Message> {
        let (total_dur, total_size) = self
            .files
            .iter()
            .fold((0u64, 0u64), |(d, s), f| (d + f.duration_secs, s + f.size_bytes));
        ...
        row![
            text(selected).size(11).color(MUTED).width(Length::Fixed(220.0)),
...
fn load_full(path: &Path) -> (TagForm, Option<Id3v1Display>, String, Option<CoverInfo>) {
    ...
    if let Some(tag) = editable_tag(&tagged_file) {
        ...
        form.track_total = tag
            .track_total()
            .map(|t| t.to_string())
            .unwrap_or_default();
        form.disc = tag.disk().map(|d| d.to_string()).unwrap_or_default();
        form.disc_total = tag.disk_total().map(|d| d.to_string()).unwrap_or_default();
...
fn save_tags(path: &Path, form: &TagForm) -> Result<(), String> {
    ...
    set_or_remove_string(&mut tag, ItemKey::Genre, &form.genre, |t, v| t.set_genre(v));
...
impl OpusSource {
    ...
    fn fill(&mut self) -> bool {
        ...
        loop {
            ...
            let mut buf = vec![0.0f32; MAX_SAMPLES_PER_CHANNEL * self.channels as usize];
            let decoded = match self.decoder.decode_float(&packet.data, &mut buf, false) {
                Ok(n) => n,
                Err(e) => {
            ...
            if self.pre_skip_remaining > 0 {
                let skip_frames =
                    (self.pre_skip_remaining as usize).min(decoded);
